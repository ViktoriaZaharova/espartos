...
function mark_in_basket(element_id, quantity){
    ...
    var sizes_variants_selector = document.getElementById('product_size_selector_wrp');
    ...
    if(typeof to_basket_button.dataset !== 'undefined' && to_basket_button.dataset['hasSizesVariants'] && sizes_variants_selector){
        sizes_variants_selector.className += " disabled";
    }
...
function calculate_product_detail_price(){
    if(typeof window.product_base_price === 'number'
        && typeof window.product_price === 'number'
        && typeof window.product_discount_percent === 'number'
        //&& typeof window.current_element_id == 'number'
    ){
        var total_price = 0;
        var $faktura_oboev_selector = $('#faktura_oboev_carousel');
        var $product_size_selector = $('#product_size_selector_wrp');
        var $product_quantity_input = $('#quantity_selector_' + current_element_id).find('input');
        
        //if($product_quantity_input.length){
            if($faktura_oboev_selector.length){
                var $selected_faktura_item = $faktura_oboev_selector.find('.selected_item');
                
                if($selected_faktura_item.length){
                    var extra_charge = parseInt($selected_faktura_item.data('extra-charge'), 10);
                    extra_charge = isNaN(extra_charge) ? 0 : extra_charge;
                    
                    total_price += extra_charge;
                }
            }
            
            if($product_size_selector.length){
                var $selected_size_item = $product_size_selector.find('.active');
                
                if($selected_size_item.length){
                    var extra_charge = parseInt($selected_size_item.data('extra-charge'), 10);
                    extra_charge = isNaN(extra_charge) ? 0 : extra_charge;
            ...
            total_price += product_base_price;
            
            //var product_quantity_value = parseInt($product_quantity_input.val(), 10);
            //product_quantity_value = (isNaN(product_quantity_value) || product_quantity_value < 1) ? 1 : product_quantity_value;
            
            //total_price = total_price * product_quantity_value;
            
            if(product_discount_percent){
                var total_discount_price = Math.floor(total_price * ((100 - product_discount_percent) / 100));
            }else{
                var total_discount_price = total_price;
            }
            
            if(total_discount_price <= 0){
                total_discount_price = product_price;
            }
            
            var total_price_text_value = number_format(total_price, 0, '', ' ');
            var total_discount_price_text_value = number_format(total_discount_price, 0, '', ' ');
            
            //console.log('Product total price: ' + total_price + "\nProduct discount total price: " + total_discount_price + "\nProduct discount total price text: " + total_discount_price_text_value);
            
            $('.product_old_price_value_holder').text(total_price_text_value);
            $('.product_price_value_holder').text(total_discount_price_text_value);
        //}
    }
}
...
$(document).ready(function(){
	/* ↓ SHOW_DESIGNER ↓ */
	$('.show_designer_change_btn').click(function(){
		$(this).closest('.show_designer_change_btn_wrp').slideUp(300).siblings('.designer_change_wrp').slideDown(300);
	});
	/* ↑ SHOW_DESIGNER ↑ */
	
    /* ↓ DROPDOWN_MENU ↓ */
    $('.dropdown_catalog_menu_wrp').find('.catalog_list_dropdown_menu_btn').click(function(event){
        event.preventDefault();
        event.stopPropagation();
        $(this).closest('.dropdown_catalog_menu_wrp').toggleClass('opened');
    });
    $('body').click(function(){
        $('.dropdown_catalog_menu_wrp').removeClass('opened');
    });
    /* ↑ DROPDOWN_MENU ↑ */

    /* ↓ FAST_LINKS ↓ */
    //$('.fast_links_list').find('.show_hide_btn.show').click(function(){
    $('body').on('click', '.fast_links_list .show_hide_btn.show', function(){
        $(this).parent().addClass('visible');
    }).on('click', '.fast_links_list .show_hide_btn.hide', function(){
        $(this).parent().removeClass('visible');
    });
    /* ↑ FAST_LINKS ↑ */
    
    if(typeof $.fn.perfectScrollbar !== 'undefined'){
        $('.scrollbar_wrp').perfectScrollbar();
    }
    ...
    */
    ...
    $('.cities_open_btn').click(function(event){
        ...
        var $mobile_cities_wrp = $('.mobile_cities_wrp');
        
        $mobile_cities_wrp.addClass('opened').css({'height':body_height-64});
        
        if(!$mobile_cities_wrp.hasClass('loaded') && !$mobile_cities_wrp.hasClass('loading')){
            console.log('Gonna load mobile cities list');
            $mobile_cities_wrp.addClass('loading');
            $mobile_cities_wrp.find('.cities_list_wrp').addClass('preloader');
            
            $.ajax({
                'url': '/ajax/get_mobile_cities_list.php',
                'type': "GET",
                'cache': false,
                'context': $this,
                'data': {},
                ...
                if(typeof data === 'object'){
                    if(data.result == 'success'){
                        var $mobile_cities_wrp = $('.mobile_cities_wrp');
                        $mobile_cities_wrp.find('.cities_list').html(data.markup);
                        $mobile_cities_wrp.addClass('loaded');
                        
                        city_link_redirect_handler({'parent_item': $mobile_cities_wrp});
                        city_link_cookie_handler({'parent_item': $mobile_cities_wrp});
                    }else{
                ...
                var $mobile_cities_wrp = $('.mobile_cities_wrp');
                $mobile_cities_wrp.removeClass('loading');
                $mobile_cities_wrp.find('.cities_list_wrp').removeClass('preloader');
            });
        }else{
            console.log('Mobile cities list has already been loaded.');
        }
    });
    ...
    $('body').on('click', '.catalog_filter_btn', function(){
        $('.filter_btn_mobile').addClass('opened').siblings('.left_menu_filter_list').addClass('opened').siblings('.filter_iner_btns').addClass('opened');
    });
    //$('.filter_btn_mobile').click(function(){
    $('body').on('click', '.filter_btn_mobile', function(){
        $(this).removeClass('opened').siblings('.left_menu_filter_list').removeClass('opened').siblings('.filter_iner_btns').removeClass('opened');
        /*
        if($(this).hasClass('opened')){
    ...
    $('body').on('click', '.catalog_sort_btn', function(){
        $(this).closest('.catalog_filter_btns_wrp').siblings('.catalog_filter_wrp').find('.catalog_filter_list').toggleClass('opened');
    });
    /* ↑ FILTER_CATALOG_MOBILE_BTN ↑ */
    ...
    $('.detail_wrp').on('click', 'button.to_basket_btn', function(){
        ...
        var has_sizes_variants_selector = $this.data('has-sizes-variants') ? true : false;
        var faktura_basket_param_name = '';
        ...
        if(element_id){         
            ...
            if(has_faktura_selector){
                ...
                if($faktura_oboev_wrp.length && $faktura_oboev_items.length){
                    ...
                    if($selected_faktura_item.length){
                        product_params_data.push('faktura=' + $selected_faktura_item.data('element-id'));
                    }else{
                        $faktura_oboev_wrp.find('.error_text_wrp').slideDown('fast');
                        return false;
                    }
                    
                    var param_name_from_block = $faktura_oboev_wrp.data('basket-param-name');
                    
                    if(param_name_from_block && typeof param_name_from_block === 'string' && $.trim(param_name_from_block)){
                        faktura_basket_param_name = $.trim(param_name_from_block);
                    }
                }
            }
            
            if(has_sizes_variants_selector){
                var $sizes_variants_selector_wrp = $('.product_size_selector_wrp');
                var $sizes_variants_items = $sizes_variants_selector_wrp.find('.size_variant_item');
                
                if($sizes_variants_selector_wrp.length && $sizes_variants_items.length){
                    var $selected_size_variant_item = $sizes_variants_selector_wrp.find('.active');
                    
                    if($selected_size_variant_item.length){
                        product_params_data.push('size_variant=' + $selected_size_variant_item.data('element-id'));
                    }else{
                        $sizes_variants_selector_wrp.find('.error_text_wrp').slideDown('fast');
                        return false;
                    }
                }
            }
            ...
            $.ajax({
                ...
                'cache': false,
                'context': $this,
                'data': {'action': 'add_to_basket', 'element_id': element_id, 'quantity': quantity_value, 'params': product_params_data, 'faktura_basket_param_name': faktura_basket_param_name},
            }).done(function(data){
                var $this = $(this);
                var element_id = $this.data('id');
                var has_faktura_selector = $this.data('has-faktura') ? true : false;
                ...
                if(data.STATUS == 'OK'){
                    ...
                    if(has_sizes_variants_selector){
                        ...
                        if($sizes_variants_selector_wrp.length){
                            $sizes_variants_selector_wrp.addClass('disabled');
                        }
                    }
    ...
    */
    ...
    function show_popup_btn_click_handler(event){
        event.preventDefault();
        var body_width = $('body').outerWidth();
        $('body').addClass('overflow_hide').css({'width':body_width});
        $('.popup_wrp').fadeIn(300).addClass('overflow_scroll');
    ...
    window.show_popup_btn_click_handler = show_popup_btn_click_handler;
    
    $('.popup_click').click(show_popup_btn_click_handler);
    
    $('.closing_click').click(function(event){
        var block_object = $(event.target);
        if(block_object.hasClass('closing_click')){
            var $popup_wrp = $('.popup_wrp');
            var $delivery_popup_wrp = $('.delivery_popup_wrp');
            
            if($popup_wrp.length){
                $popup_wrp.fadeOut(500).removeClass('overflow_scroll').find('.popup').fadeOut(300);
            }
            
            if($delivery_popup_wrp.length){
                $delivery_popup_wrp.fadeOut(500).removeClass('overflow_scroll').find('.popup').fadeOut(300);
            }
    ...
    /* ↓ DELIVERY_POPUP ↓ */
    $('.delivery_popup_click').click(function(event){
        event.preventDefault();
        var $this = $(this);
        
        var delivery_service_id = $this.data('id');
        var $popup_node = delivery_service_id ? $('.delivery_service_' + delivery_service_id + '_popup') : [];
        
        if($popup_node.length){
            var body_width = $('body').outerWidth();
            $('body').addClass('overflow_hide').css({'width':body_width});
            $('.delivery_popup_wrp').fadeIn(300).addClass('overflow_scroll');
            $('#scroll_up_btn, #scroll_down_btn').hide();
            
            $popup_node.fadeIn(500);
        }else{
    ...
    function city_change_btn_click_handler(event){
        console.log('Clicked');
        $('#city_change_wrp').fadeIn(500);
        event.preventDefault();
    }
    
    window.city_change_btn_click_handler = city_change_btn_click_handler;
    
    $('#city_change_btn, .city_change_btn').click(city_change_btn_click_handler);
    
    function city_link_redirect_handler(params){
        var $parent_item = false;
        var parent_selector = '#city_change_wrp, #city_change_mobile_wrp';
        
        if(typeof params === 'object'){
            if(params.parent_selector){
        ...
        if(!$parent_item){
            $parent_item = $(parent_selector);
        }
        
        $parent_item.find('.cities_list a').click(function(e){
            e.preventDefault();
    ...
    city_link_redirect_handler();
    
    function city_link_cookie_handler(params){
        var $parent_item = [];
        ...
        if(typeof params !== 'undefined'){
            if(params.parent_item){
                ...
                $parent_item = $(params.parent_selector);
            }
        }
        
        var $items = $parent_item.length ? $parent_item.find('.cities_list a') : $('.cities_list a');
    ...
    // ↓ TABS ↓
    $('body').on('click', 'ul.tabs_list li', function(){
    ...
    /* ↓ CAROUSELS ↓ */
	$('.element_comment_slider').owlCarousel({
        items : 1,
        singleItem : true,
        navigation : false,
        slideSpeed : 600,
    ...
    $('#faktura_oboev_carousel').find('.select_btn').click(function(){
        ...
        calculate_product_detail_price();
    });
    ...
    $('.new_collections_slider').owlCarousel({
        items : 4,
        singleItem : false,
        navigation : true,
        slideSpeed : 600,
    ...
    */
    ...
    /* ↓ ELEMENT_SIZE_CHECK ↓ */
    $('.other_sizes_size').click(function(){
        //$(this).parent().toggleClass('active').siblings().removeClass('active').parent().siblings().children().removeClass('active');
        var $this = $(this);
        var $wrp_element = $this.closest('.product_size_selector_wrp');
        
        if(!$wrp_element.hasClass('disabled')){
            $wrp_element.find('.active').removeClass('active');
            $this.parent().addClass('active');
            
            $wrp_element.find('.error_text_wrp').slideUp('fast');
    ...
    $('body').on('click', '.self_photo_wallpaper_btn', function(){
    ...
    var fast_search_ajax_timeout = false;
    
    function request_fast_search_results(query){
        if(query){
            $('#fast_search_ajax_holder').addClass('preloader').show();
            
            $.ajax({
                'url': '/ajax/search_field_request.php',
                'type': "GET",
                'cache': false,
                'data': {'s': query},
            }).done(function(data){
                if(data){
                    $('#fast_search_ajax_holder').html(data).removeClass('preloader').show();
                }else{
                    $('#fast_search_ajax_holder').hide();
                }
            }).fail(function(){
                $('#fast_search_ajax_holder').hide();
            }).always(function(){
    ...
    $('.menu_green .search_wrp, .white_search_wrp .search_dropdown_wrp').find('input[name="q"]').keyup(function(){
        var $this = $(this);
        var search_request = $this.val().replace(/^\s+|\s+$/g, '');
        
        clearTimeout(fast_search_ajax_timeout);
        
        console.log('Search: ' + search_request);
        
        if(search_request && search_request.length > 1){
            fast_search_ajax_timeout = setTimeout(function(){ request_fast_search_results(search_request); }, 500);
        }else{
            $('#fast_search_ajax_holder').hide();
        }
    });
...
$(window).load(function(){
    $('.slider_banner_wrp').addClass('height_auto');

    $('.dropdown_catalog_menu_wrp').find('.dropdown_catalog_menu_content_wrp').removeClass('hidden');
