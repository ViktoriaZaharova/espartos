...
function calculate_wallpaper_height(){
	...
	if($wallpaper_info_block.length){
		...
		$('.detail_wrp .main_info_wrp .wallpaper_image_wrp').css({'height':block_height});
	}
}
...
$(document).ready(function(){
	...
	$('.detail_wrp').on('click', 'button.to_basket_btn', function(){
		...
		if(element_id){
			...
			$.ajax({
				'url': '/new/ajax/add_to_basket.php',
				'type': "POST",
				'cache': false,
				'context': $this,
				'data': {'action': 'add_to_basket', 'element_id': element_id, 'quantity': quantity_value},
			}).done(function(data){
				if(data.STATUS == 'OK'){
	...
	*/
	...
	$('.wallpapers_by_purpose_filter').on('click', '.purpose_item', function(){
		if(!window.purpose_filter_active){
			...
			$('#wallpapres_by_destination_list_wrp').addClass('loading');
			
			$.get("/new/ajax/get_wallpapers_by_destination.php?destination_id=" + purpose_id, function(data) {
				$('#wallpapres_by_destination_list_wrp').html(data).removeClass('loading');
